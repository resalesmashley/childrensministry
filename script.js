...
function handlePageNavigationClick(event) {
    ...
    const isSubNav = link.closest('.sub-nav');
    const isOverviewLink = link.classList.contains('overview-link') || link.closest('.overview-tiles');
    if (!isSubNav && !isOverviewLink) return;
...
function initializeSubNavigation() {
    ...
    subNav.addEventListener('click', handlePageNavigationClick);
    subNav.dataset.bound = 'true';
}

function initializeOverviewNavigation() {
    const overviewTiles = document.querySelector('.overview-tiles');
    if (!overviewTiles || overviewTiles.dataset.bound === 'true') return;

    overviewTiles.addEventListener('click', handlePageNavigationClick);
    overviewTiles.dataset.bound = 'true';
}
